...
lollipop <- function(fin,outf,domains,maxlen,type_len){
    ...
    vars<-vars[order(vars$Group,decreasing = T),]
  ...
  #  pointcols<-c(rgb(col1[1],col1[2],col1[3],alpha = 0.5),rgb(0,0,139/255,0.5),rgb(col4[1],col4[2],col4[3],0.5),rgb(col2[1],col2[2],col2[3],0.5))
    ...
    for(group in toupper(unique(vars$Group) )){
       ...
       # text(-10,direction*width*3,labels = str_to_title(group),cex=cfont)
    ...
    for(i in 1:dim(domains)[1]){
      ...
      text(as.numeric(domain$From)+(as.numeric(domain$To)/2-(as.numeric(domain$From))/2),center,
           labels = domain$Domain,col="black",cex=1.5*cfont)
  ...
  #  axis(1,at=c(0,33,131,201,501,1038),labels = c(0,33,131,201,501,"1038 aa."),line = -3,tck=-0.02,col="gray",col.axis="gray")
    ...
    text(maxlen+12,y = center,labels="aa.",srt=90,cex=cfont*0.85,col="black",font=2)
    legend(maxlen-400,-2,legend = fmut_types[1:type_len],
          horiz = T,pch=19,col=pointcols[1:type_len],cex=cfont*1.2,bty='n',pt.cex = pcex,y.intersp = 1.2)
    dev.off()
}
...
outf<-"PAH/Result/Data/output/BMPR2_lolliplot.pdf"
...
# maxlen=545
# border_col<-"gray"
# domain_col<-c("#FF8833","#51C6E6","yellow","red","orange")
# domains<-as.data.frame(rbind(c("T-BOX",66,252)),stringsAsFactors = F)
# names(domains)<-c("Domain","From","To")
# fin<-"~/Dropbox/CUMC/PAH/FInal/TBX4_plot.txt"
# outf<-"~/Dropbox/CUMC/PAH/FInal/TBX4_lolliplot.pdf"
