...
formatFreq_new<-function(data){
  if(length(grep("AF",names(data)))>0){
    data$AF[which(is.na(as.numeric(data$AF)))]=0
  }
  bravo_index<-grep("BRAVO_.*AF",names(data),ignore.case = T)
  if(length(bravo_index)>0){
    data[which(is.na(as.numeric(data[,bravo_index]))),bravo_index]=0
    data[which((as.numeric(data[,bravo_index])==-1)),bravo_index]=0
  }
...
load_dataset<-function(){
 ...
 # fexac<-paste(resources,"ExAC_r03_0316_z_pli_rec_null_data.bed",sep="")
  ...
  fendothelial<-paste(resources,"GeneExpression/adult_Endothelial_gene.avg.addname.rank.csv",sep="")
  fsmc<-paste(resources,"GeneExpression/SMC_pulmonary_artery_Replicate1.addname.rank.tsv",sep="")
  #fendothelial<-paste(resources,"GeneExpression/adult_Endothelial_gene.replicate1.csv.rank.tsv.addname.tsv",sep="")
 ...
 # exac<-read.table(fexac,stringsAsFactors = F,header = 1,check.names = F,comment.char = "")
  heart_exp<-read.csv(fheart,sep = ",",header = 1,check.names = F,stringsAsFactors = F)
  lung_exp<-read.csv(flung,sep=",",header = 1,check.names = F,stringsAsFactors = F)
  enth_exp<-read.csv(fendothelial,header = 1,check.names = F,stringsAsFactors = F)
  diaphragm_exp<-read.csv(fdiaphram,sep=",",header = 1,check.names = F,stringsAsFactors = F)
  SMC_exp<-read.table(fsmc,header = 1,check.names = F,stringsAsFactors = F)
 
  return(list(heart_exp=heart_exp,lung_exp=lung_exp,diaph_exp=diaphragm_exp,endothelial_exp=enth_exp, smc_exp=SMC_exp))
}
load_exac<-function(){
  resources="Resources/"
...
gene_exp_diaphragm<-function(data,lib){
  for(id in 1:dim(data)[1]){
    ...
    if(length(index)>0){
      data$diaphragm_rank[id]=100-lib$rank[index]
...
gene_exp_endothelial<-function(data,lib){ ## reversed
  gid<-which(names(data)=="Gene.refGene"|names(data)=="GeneName")[1]
 # ranks<-unlist(lapply(data[,gid],FUN = function(x){index<-grep(x,lib$GeneName)[1];  if(length(index)>0){return(lib$Rank[index]*100)}else{return(0)} }))
#  data$Endothelial<-ranks
  for(id in 1:dim(data)[1]){
    gid<-which(names(data)=="Gene.refGene"|names(data)=="GeneName")[1]

    index<-grep(data[id,gid],lib$GeneName)[1]
    if(length(index)>0){
      data$Endothelial[id]=lib$Rank[index]*100
      #  data$diaphragm_E115[id]=lib$E11.5[index]
    }else{
      data$Endothelial[id]="."
      #data$diaphragm_E115[id]="."
  ...
  print("Endothelial Expression, the higher the more related")
  return(data)
}
...
gene_exp_SMC<-function(data,lib_exp){ ## reversed
  gid<-which(names(data)=="Gene.refGene"|names(data)=="GeneName")[1]
  # ranks<-unlist(lapply(data[,gid],FUN = function(x){index<-grep(x,lib$GeneName)[1];  if(length(index)>0){return(lib$Rank[index]*100)}else{return(0)} }))
  #  data$Endothelial<-ranks
  for(id in 1:dim(data)[1]){
    ...
    index<-grep(data[id,gid],lib_exp$GeneName)[1]
    if(length(index)>0){
      data$SMC[id]=lib_exp$Rank[index]*100
      #  data$diaphragm_E115[id]=lib$E11.5[index]
    }else{
      data$SMC[id]="."
      #data$diaphragm_E115[id]="."
  ...
  print("SMC plumonary arterial Expression, the higher the more related")
  return(data)
}
...
string_vector<-function(var_ann,gts,case_list,control_list){
  full_list<-names(var_ann)
  nms<-full_list[grep("chrom|FROM|REF|ALT",full_list,invert = T,ignore.case = T)]   ##,"QUAL","FITER")#,"INFO","FORMAT",probands)
  str=paste(var_ann[,"#CHROM"],var_ann[,"FROM"],var_ann[,"TO"],var_ann[,"REF"],var_ann[,"ALT"],sep="\t");
  str=paste(str,"100",sep="\t")
  str=paste(str,var_ann[,"FILTER"],sep="\t")
  info<-"";
  for(nm in c("ANNO","ExonicFunc.refGene","AAChange.refGene")){
    info=paste(info,nm,"=",var_ann[,nm],";",sep="")
  }
  str=paste(str,info,"GT",sep="\t")
  str_gt="";
  for(g in gts){
    str_gt<-paste(str_gt,g,sep="\t")
  }
  str=paste(str,str_gt,sep="")#,sep="\t")
  return(str)
}


vcf_output<-function(dat,dat2,case_list,control_list){
  out<-c();
  header<-"#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t";
  for(id in c(case_list,control_list)){
    header<-paste(header,id,sep="\t")
  }

  vars<-unique(rbind(dat[,c('#CHROM',"FROM","REF","ALT")],dat2[,c('#CHROM',"FROM","REF","ALT")]))
  vars<-vars[order(vars$FROM),]
  for(i in 1:dim(vars)[1]){
    var=vars[i,];
    gts<-rep("0/0",length(case_list)+length(control_list))
    index<-which(dat$`#CHROM`==var$`#CHROM` & dat$FROM==var$FROM & dat$REF==var$REF & dat$ALT==var$ALT)
    if(length(index)>0){
      hits<-unlist(lapply(index,FUN = function(x){i=which(case_list==dat$ProbandName[x]);}))
      gts[hits]<-unlist(lapply(dat$Genotype[index],FUN = function(x){unlist(strsplit(x,split = ":",fixed = T))[1]}))
      var_ann<-dat[index,]
    }
    index2<-which(dat2$`#CHROM`==var$`#CHROM` & dat2$FROM==var$FROM & dat2$REF==var$REF & dat2$ALT==var$ALT)
    if(length(index2)>0){
    #  print(c(dat2$Hom_NFE[index2]))
      sim_gts<-c(rep(0,length(control_list)-dat2$AC_NFE[index2] +dat2$Hom_NFE[index2]) ,rep(1,dat2$AC_NFE[index2]-2*dat2$Hom_NFE[index2]),rep(2,dat2$Hom_NFE[index2]))
      sim_gts<-sample(sim_gts,size = length(control_list),replace = F)
      gts[which(sim_gts==1)+length(case_list)]<-"0/1"
      gts[which(sim_gts==2)+length(case_list)]<-"1/1"
      var_ann<-dat2[index2,]
    }
    
    str<-string_vector(var_ann,gts,case_list,control_list)
    #print(str)
    out<-c(out,str)
  ...
  names(out)<-header;
  return(out)
#  write.csv(out,file="PAH/PAH_10032017/Result/transfer.vcf",row.names = F,quote = F)
}
