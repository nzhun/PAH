library(stringr)
lollipop <- function(fin,outf,domains,maxlen,type_len,gene){
    fold=1
    folder=1 #.4 
    dev=5
    fdev=4
  ...
  #  pointcols<-c(rgb(col1[1],col1[2],col1[3],alpha = 0.5),rgb(0,0,139/255,0.5),rgb(col4[1],col4[2],col4[3],0.5),rgb(col2[1],col2[2],col2[3],0.5))
    ...
    for(group in c(toupper(unique(vars$Group) ),"")){
      print(group)
      if(is.na(group)){next}
      direction<- 1
      
      if(group==""){
        pos<-vars[which(vars$Gene==gene),]
      
      }else{
        pos<-vars[which(toupper(vars$Group)==group & vars$Gene==gene),]  
      }
    
       par(mar=c(0,0,0,0),mai=c(0,0,0,0),oma=c(0,0,0,0))
       plot(0,1,xlim=c(0,maxlen),ylim=c(-3.5,3.5),xlab="",ylab="", type='n',bty='n',axes=F,main="")
       title(main=group,line=-5)
        ## lines
        ...
        for(PRC in unique(pos$AA)){
         ...
         #   offset<-pos$offset[j]
            ...
            if(length(grep("D-MIS",types,ignore.case = T))>0){
              #if(pos$MetaSVM[j]=="D"){
              ...
              #  pointcol=pointcols[4]
               # next;
              #}
            }else if(length(grep("inframe|in_frame|in-frame",types,ignore.case = T))>0){
        ...
        text(7,direction*width*2.5,labels = toupper(gene),cex=cfont)
...
#file="PAH/PAH_10032017/Doc/knowgeneforLolliplot.txt"
file="PAH/PAH_10032017/src/PAH_known.candidates.0607.lolliplot.txt"
#data<-read.table(file,header = 1,comment.char = "",stringsAsFactors = F,sep="\t")
#data$AA<-unlist(lapply(1:dim(data)[1],FUN = function(x) gsub(unlist(strsplit(data$ProteinChange[x],split = "-"))[1],pattern = "[a-z.]",replacement = "",ignore.case = T,fixed = F)))
#write.table(data,file,row.names = F,quote = F,sep="\t")
outf<-"PAH/PAH_10032017/Doc/knowngene.pdf"
pdf(outf,width=15,height=4)
gene="TBX4"
maxlen=545
border_col<-"gray"
...
names(domains)<-c("Domain","From","To")
fin<-file #"PAH/Result/Data/source/TBX4_plot.txt"
#outf<-"PAH/PAH_10032017/Doc/TBX4.pdf" #"PAH/Result/Data/output/TBX4_lolliplot.pdf"
lollipop(fin,outf,domains,maxlen,type_len=3,gene)
...
border_col<-"gray"
domain_col<-c("green","#51C6E6","yellow","red","orange")
domains<-as.data.frame(rbind(c("HMG_BOX",61,141),c("Sox_C_TAD",199,412)),stringsAsFactors = F)
names(domains)<-c("Domain","From","To")
#outf<-"PAH/PAH_10032017/Doc/TBX4.pdf" #"PAH/Result/Data/output/TBX4_lolliplot.pdf"
lollipop(fin,outf,domains,maxlen,type_len=3,gene)
...
border_col<-"gray"
domain_col<-c("green","#51C6E6","yellow","red","orange")
domains<-as.data.frame(rbind(c("ABC_Mem",300,590),c("ABC_tran",700,900),c("ABC_mem",1050,1300),c("ABC_tran",1350,1500)),stringsAsFactors = F)
names(domains)<-c("Domain","From","To")
#outf<-"PAH/PAH_10032017/Doc/TBX4.pdf" #"PAH/Result/Data/output/TBX4_lolliplot.pdf"
lollipop(fin,outf,domains,maxlen,type_len=3,gene)
...
border_col<-"gray"
domain_col<-c("green","#51C6E6","yellow","red","orange")
domains<-as.data.frame(rbind(c("TGF",180,200),c("Pkinase",210,490)),stringsAsFactors = F)
names(domains)<-c("Domain","From","To")
#outf<-"PAH/PAH_10032017/Doc/TBX4.pdf" #"PAH/Result/Data/output/TBX4_lolliplot.pdf"
lollipop(fin,outf,domains,maxlen,type_len=3,gene)
...
border_col<-"gray"
domain_col<-c("green","#51C6E6","yellow","red","orange")
domains<-as.data.frame(rbind(c("Caveolin",40,175)),stringsAsFactors = F)
names(domains)<-c("Domain","From","To")
#outf<-"PAH/PAH_10032017/Doc/TBX4.pdf" #"PAH/Result/Data/output/TBX4_lolliplot.pdf"
lollipop(fin,outf,domains,maxlen,type_len=3,gene)
...
border_col<-"gray"
domain_col<-c("green","#51C6E6","yellow","red","orange")
domains<-as.data.frame(rbind(c("Zona",350,575)),stringsAsFactors = F)
names(domains)<-c("Domain","From","To")
#outf<-"PAH/PAH_10032017/Doc/TBX4.pdf" #"PAH/Result/Data/output/TBX4_lolliplot.pdf"
lollipop(fin,outf,domains,maxlen,type_len=3,gene)
...
maxlen=394
border_col<-"gray"
domain_col<-c("green","green")
domains<-as.data.frame(rbind(c("Ion_trans_2",55,135),c("Ion_trans_2",165,245)),stringsAsFactors = F)
names(domains)<-c("Domain","From","To")
#outf<-"PAH/PAH_10032017/Doc/TBX4.pdf" #"PAH/Result/Data/output/TBX4_lolliplot.pdf"
lollipop(fin,outf,domains,maxlen,type_len=3,gene)
...
maxlen=552
border_col<-"gray"
domain_col<-c("green","pink")
domains<-as.data.frame(rbind(c("MH1",35,135),c("MH2",320,525)),stringsAsFactors = F)
names(domains)<-c("Domain","From","To")
#outf<-"PAH/PAH_10032017/Doc/TBX4.pdf" #"PAH/Result/Data/output/TBX4_lolliplot.pdf"
lollipop(fin,outf,domains,maxlen,type_len=3,gene)
...
border_col<-"gray"
domain_col<-c("green","pink")
domains<-as.data.frame(rbind(c("MH1",35,135),c("MH2",270,445)),stringsAsFactors = F)
names(domains)<-c("Domain","From","To")
#outf<-"PAH/PAH_10032017/Doc/TBX4.pdf" #"PAH/Result/Data/output/TBX4_lolliplot.pdf"
lollipop(fin,outf,domains,maxlen,type_len=3,gene)
...
maxlen=1649
border_col<-"gray"
domain_col<-c("green","#51C6E6","#51C6E6","#51C6E6","orange","yellow","red")
domains<-as.data.frame(rbind(c("RWD",10,135),c("PKinase",300,500),c("PKinase",590,630),c("PKinase",790,1000),c("tRNA-synt_HITS",1050,1380),c("HGTP",1400,1649)),stringsAsFactors = F)
names(domains)<-c("Domain","From","To")
#outf<-"PAH/PAH_10032017/Doc/TBX4.pdf" #"PAH/Result/Data/output/TBX4_lolliplot.pdf"
lollipop(fin,outf,domains,maxlen,type_len=3,gene)
