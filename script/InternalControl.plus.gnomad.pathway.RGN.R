...
#start<-function(){
  ...
  #  result<-process(total_case,total_cmb,PAH_CHD_dat,"PAH_Cin_hg38_all_RGN_Inhouse_plus_gnomad",chr_vars) #,"ALL","ALL")
   #result<-process(total_case,total_cmb,auto_pah,"PAH_Cin_hg38_Auto_0.15_Inhouse_plus_gnomad_pathway",auto_gnomad) #,"ALL","ALL")
   
   fd="PAH/PAH_10032017/Result/"
  for(list in c("Portopulmonary.pediatrics.CCHMC.list",
                "PAH.CCHMC.list",
                "IPAH.CCHMC.list",
                "APAH.CCHMC.list",
                
                "Portopulmonary.CCHMC.list",
        
                "Female.CCHMC.list",
                "Male.CCHMC.list",
                "Pediatric.CCHMC.list",
                "CHD.CCHMC.list",
           #     "CHD.female.CCHMC.list",
          #      "CHD.male.CCHMC.list",
                "CHD.pediatrics.CCHMC.list",
                "CTD.CCHMC.list",
           #     "CTD.female.CCHMC.list",
           #     "CTD.male.CCHMC.list",
               # "CTD.pediatrics.CCHMC.list",
           #     "IPAH.female.CCHMC.list",
          #      "IPAH.male.CCHMC.list",
                "IPAH.pediatrics.CCHMC.list"
                #"CCHMC.female.remaining.list",
                #"CCHMC.male.remaining.list",
                #"CCHMC.pediatrics.remaining.list",
                #"CCHMC.remaining.list",
                #"CHD.remaining.list",
                #"CHD.female.remaining.list",#,
                #"CHD.male.remaining.list",
                #"CHD.pediatrics.remaining.list",
                #"IPAH.pediatrics.remaining.list",
                #"IPAH.male.remaining.list",
                #"IPAH.female.remaining.list",
                #"CTD.male.remaining.list",
                #"CTD.female.remaining.list",
                #"CTD.pediatrics.remaining.list","CTD.remaining.list",
                #"IPAH.remaining.list"
                )){  ##"PAH.CCHMC.list","IPAH.CCHMC.list","APAH.CCHMC.list","Portopulmonary.CCHMC.list",
    ## IPAH
   print(list)
   sub_ids<-read.csv(paste(fd,list,sep=""),skip = 1,stringsAsFactors=F)
   sub_ids<-sub_ids[,1]
   total_case<-length(sub_ids);
   result<-process(total_case,total_cmb,PAH_CHD_dat[which(auto_pah$ProbandName %in% sub_ids),],paste(list,"_Inhouse_plus_gnomad_pathway",sep=""),chr_vars) 
#   result<-process(total_case,total_cmb,auto_pah[which(auto_pah$ProbandName %in% sub_ids),],paste(list,"_Inhouse_plus_gnomad_autosome_pathway",sep=""),auto_gnomad) #,"ALL","ALL")
  }
#}
#start()
   #plots()
  ...
  # PAH_CHD_dat
   ...
   write.table(rbind(PAH_CHD_dat[,nm],chr_vars[,nm]),
  file="PAH/PAH_10032017/hg38/merged.filter.CCHMC.Internal.plus.gnomad.0503.bed",sep="\t",row.names=F,quote=F)
