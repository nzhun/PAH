...
process<-function(total_case,total_control,xGen_PAH,outname,chr_vars){
  ...
  if(!is.null(sbinom)){
    sbinom$GeneSize<-paste(length(genesets),group)
    sbinom$GeneSetID<-"ALL"
    sbinom$GeneSetName<-"ALL"
    sbinom$GeneList<-"ALL"
  ...
  genefamily<-read.table("Resources/Gene/Homology_HGNC_gene_family_region.bed",stringsAsFactors = F,sep="\t",quote="",header=1)
  for(i in 1:dim(genefamily)[1]){
    genesets=unlist(strsplit(genefamily$GeneList[i],split = "|",fixed=T))
    sbinom<-test_geneset(total_case,total_control,xGen_PAH,chr_vars,genesets,correct)
    if(!is.null(sbinom)){
      sbinom$GeneSize<-genefamily$Length[i]
      sbinom$GeneSetID<-genefamily$Gene_family_ID[i]
      sbinom$GeneSetName<-genefamily$Gene_family_Name[i]
      sbinom$GeneList<-genefamily$GeneList[i]
      All_genes_binom<-rbind(All_genes_binom,sbinom)
    }
  }

  write.csv(All_genes_binom,file = paste("PAH/PAH_10032017/Result/CCHMC_Internalcontrol_plus_gnomad/",outname,".gnomeAD.protein_homology.binom.csv",sep=""),row.names = F)
...
d<- read.csv("PAH/PAH_10032017/Result/CCHMC_Internalcontrol_plus_gnomad/PAH_Cin_hg38_all_RGN_Inhouse_plus_gnomad.gnomeAD.protein_homology.binom.csv",header = 1,stringsAsFactors = F,check.names = F)
subid<-which(d$Pvalue<0.05 & d$OR>=1.2 & d$GeneSetID!=346 )
ds<-d[subid,]
ds$OR[which(ds$OR==Inf)]<-30
md<-data.frame(cbind(as.numeric(unique(ds$GeneSetID)),1:length(unique(ds$GeneSetID))),stringsAsFactors = F)

names(md)<-c("ID","seq")
ds<-merge(ds,md,by.x = "GeneSetID",by.y="ID")
ramp <- colorRamp(c("red", "blue"))
cols<-rgb( ramp(seq(0, 1, length = 20)), max = 255)
pchs<-c(3,4,8,20)

pdf("~/Desktop/Protein_homology.pdf",width=11,height=11)
plot(0,1,xlim=c(1,max(md$seq)),ylim=c(0,max(ds$OR)*1.2),type='n',ylab="Enrichment",xlab="Gene Family ID",xaxt='n');
axis(1,at = md$seq[seq(1,dim(md)[1],by = 40)],labels = md$ID[seq(1,dim(md)[1],by = 40)],las=2)

j=1;
for(t in unique(ds$seq)){
  sds<-ds[which(ds$seq==t),]
  sds<-sds[order(sds$OR,decreasing = T)[1],]
  print(sds[,c("Pvalue","OR","GeneSetID","seq")])
  points(t,sds$OR,col="black",pch=pchs[ceiling(-log10(sds$Pvalue))])
  if(sds$Pvalue <0.01  & sds$OR>=2 & sds$GeneSetID!=346 | (sds$Pvalue<0.05 & sds$OR>5 & sds$GeneSetID!=346)){
    text(t,sds$OR,labels = sds$GeneSetName,cex=0.7,srt=-90,offset =0,pos=3,col="red")
  }
  j=j+1;
}


#legend("top",legend = unique(ds$Type),fill=cols,cex=0.5)
legend("topright",legend = paste("E-",1:4,sep=""),pch=pchs,col=1,title = "pvalue")
dev.off()
